            ...
            <div class="card shadow-sm mb-4">
                ...
                <div class="card-body">
                    <table class="table table-borderless">
                        ...
                        <tr>
                            <th>Kode Buku</th>
                            <td>: <code class="fs-6"><?= esc($buku['kode_buku']) ?></code></td>
                        </tr>
                        <tr>
                            <th>Penulis</th>
                        ...
                        <tr>
                            ...
                            <td>:
                                <?php if ($buku['nama_kategori']): ?>
                                    <span class="badge bg-info"><?= esc($buku['nama_kategori']) ?></span>
                                <?php else: ?>
                    ...
                    <p class="text-justify mt-3">
                        <?= $buku['deskripsi'] ? esc($buku['deskripsi']) : '<i class="text-muted">Tidak ada deskripsi tersedia.</i>' ?>
                    </p>
                </div>
            </div>
        </div>
            ...
            <div class="card shadow-sm mb-4">
                ...
                <div class="card-body text-center">
                    <div class="mb-3">
                        ...
                        <span class="display-4 fw-bold text-<?= $buku['stok'] > 5 ? 'success' : ($buku['stok'] > 0 ? 'warning' : 'danger') ?>">
                            <?= $buku['stok'] ?>
                        </span>
                        <p class="mt-2">
                            <?php if($buku['stok'] > 5): ?>
                    ...
                    <div class="d-grid gap-2 mt-4">
                        <a href="<?= base_url('buku/edit/'.$buku['id']) ?>" class="btn btn-warning text-white">
                            <i class="bi bi-pencil-square"></i> Edit Data
                        </a>
                        <button type="button" class="btn btn-outline-danger" onclick="if(confirm('Hapus buku ini?')) window.location.href='<?= base_url('buku/hapus/'.$buku['id']) ?>'">
